# FAST v3.3
Flexible Assistant Toolkit for Gamemaker Studio 2 - GML is a simple language with a lot of built-in features that make it accessible to new and advanced users. However, there are still many unexpectedly difficult tasks, such as file or resolution handling, that don't quite match up with how easy other features can be. FAST is a lightweight library that provides easy-to-use tools for developers who want to focus on building their game, that emphasizes being adaptable and extensible if it doesn't *quite* do what you need.  It consists of a lightweight Core module and a single object to power out-of-the-box features such as universal mouse, keyboard and gamepad support, automatic resolution handling, and saving/loading files. For more advanced developers, everything in FAST is built by extending and implementing the FAST Core, providing common implementation patterns and data structures to compliment the built-in GML features. Because all of FAST's components are built on the Core module, anyone can easily write extensions to the FAST library with confidence their systems will work seamlessly. The library focuses on general utility and extensibility, it doesn't contain genre-specific code for example. It does contain powerful features such as databases for loading and saving complex data, or a Lua-like scripting language. In short, it's designed to compliment any game made with GMS2.3 by providing simple implementations for much requested features.
## Compatability
```
IDE     2.3.0.529
Runtime 2.3.0.401
