# FAST v3.3
Flexible Assistant Toolkit for Gamemaker Studio 2 - GML is a simple language with a lot of built-in features that make it accessible to new and advanced users. However, there are still many unexpectedly difficult tasks, such as file or resolution handling, that don't quite match up with how easy other features can be. FAST is a lightweight library that provides easy-to-use tools for developers who want to focus on building their game, and emphasizes being adaptable and extensible if it doesn't *quite* do what you need.  It begins with a lightweight "Core" module that uses only a single object, and powers out-of-the-box features such as universal mouse, keyboard and gamepad support, custom event handling, and saving/loading files. For more advanced developers, interfaces for extending and implementing the FAST Core provide common implementation patterns and data structures which compliment built-in GML workloads. All of FAST's components are built on the Core module, and anyone can quickly write extensions which use FAST, confident their systems will work seamlessly. The library does not contain genre-specific code, and focuses on general utility and adaptability. It contains powerful tools such as databases for loading and saving complex data, and even a Lua-like scripting language. However, these tools provide utility to the language rather than box it in. In short, FAST is designed to compliment any and every game made with GMS2.3 or later.
## Compatability
```
IDE     2.3.0.529
Runtime 2.3.0.401
